...
class ApprovalService:
    ...
    def create_approval_request(
        ...
        action_description = f"Execute {len(proposed_actions)} action(s) for resolution {resolution.resolution_id}"
        if proposed_actions:
        ...
        primary_action_type = "multiple_actions" if len(proposed_actions) > 1 else (
            proposed_actions[0].get('action', 'unknown') if proposed_actions else 'unknown'
        )
    ...
    def approve_request(
        ...
        self.logger.info(
            ...
            f"token {token.token[:8]}... generated"
        )
